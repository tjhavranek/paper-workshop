...
READ, using your tools. All of these are required, none optional: the manuscript at {{PAPER_TXT_PATH}};
...
- **If the paper UNDERSELLS itself, say so.** When the paper's own verified results
  ...
  `verification_status` = `needs-author-confirmation` (the author is the author, rule 14). These NEVER enter the must-fix list; they feed the chair's non-blocking
