...
- **`improvement_memo` is NON-BLOCKING and opt-in.** Compose it ONLY from
  ...
  `prioritized_findings`, never raise a severity, and never move the validity or venue verdict —
  bolder suggestions the author ratifies or ignores (rule 14). The engine caps the list to the
