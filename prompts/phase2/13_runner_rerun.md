<!-- Injected: {{FINDING_JSON}} {{EDIT_JSON}} {{CODE_DIR}} {{DATA_DIR}} {{RUN_DIR}} {{SANDBOX_NOTES_PATH}} -->
...
3. Capture the run record: exact command, stdout/stderr, environment snapshot, the
   read-only input-data hash (confirm it is unchanged after the run), and the output
   file(s) with their content hashes.
4. For each value the edit needs, emit a **provenance token**:
   ...
   The value MUST appear in the named output artifact.
