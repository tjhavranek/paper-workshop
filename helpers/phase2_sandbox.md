...
2. Diff a handful of the paper's **current** headline numbers against the fresh
   outputs.
...
- A **provenance token** = `{ value, script, line_or_chunk, run_id, input_data_hash,
  output_file, output_hash }`. The Act-II `numeric-provenance` verifier rejects any
  numeric change whose token is missing or whose value is not present in the named run
  artifact.
...
fresh, seeded, network-off environment and confirms the manuscript's numbers reproduce
(hash-match, or within tolerance for legitimately stochastic outputs). A package that
cannot reproduce its own outputs is **not** marked final — it is reported as failing.
