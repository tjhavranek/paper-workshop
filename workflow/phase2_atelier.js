...
const RUNREC = { type: 'object', additionalProperties: false, properties: { run_id: { type: 'string' }, status: { type: 'string', enum: ['ok', 'failed', 'baseline-failed'] }, command: { type: 'string' }, provenance_tokens: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { value: { type: 'string' }, script: { type: 'string' }, line_or_chunk: { type: 'string' }, run_id: { type: 'string' }, input_data_hash: { type: 'string' }, output_file: { type: 'string' }, output_hash: { type: 'string' } }, required: ['value', 'script', 'run_id', 'output_file', 'output_hash'] } }, log_excerpt: { type: 'string' } }, required: ['run_id', 'status', 'command', 'provenance_tokens', 'log_excerpt'] }
...
const RECON = { type: 'object', additionalProperties: false, properties: { reconciled: { type: 'array', items: { type: 'string' } }, orphans: { type: 'array', items: { type: 'string' } }, mismatches: { type: 'array', items: { type: 'string' } }, run_mismatches: { type: 'array', items: { type: 'string' } } }, required: ['reconciled', 'orphans', 'mismatches', 'run_mismatches'] }
const PACKAGE = { type: 'object', additionalProperties: false, properties: { manifest: { type: 'string' }, readme: { type: 'string' }, reproduced: { type: 'boolean' }, log: { type: 'string' }, labeled_gaps: { type: 'array', items: { type: 'string' } } }, required: ['manifest', 'readme', 'reproduced', 'log', 'labeled_gaps'] }
...
if (scope.blocking_gaps.length && !INPUTS.source) {
...
// ---------- Baseline gate ----------
phase('Baseline')
let baseline = { run_id: 'baseline', status: 'ok', command: '(skipped: no code provided)', provenance_tokens: [], log_excerpt: '' }
if (INPUTS.code && INPUTS.data && edits.some(e => e.lane === 'B-recompute')) {
  baseline = await agent(promptRef('phase2/13_runner_rerun', { FINDING_JSON: { note: 'BASELINE REPRODUCTION GATE — run the master script unchanged and confirm the current headline numbers reproduce.' }, EDIT_JSON: {}, CODE_DIR: INPUTS.code, DATA_DIR: INPUTS.data, RUN_DIR: (PATHS.session || '.') + '/phase2/runs/baseline', SANDBOX_NOTES_PATH: PATHS.sandbox_notes || '' }), { ...GP, label: 'baseline-gate', phase: 'Baseline', schema: RUNREC })
...
const results = (await pipeline(
  ...
  async (e) => {
    ...
    if (e.lane === 'B-recompute' || e.lane === 'C-new-analysis') {
      run = await agent(promptRef('phase2/13_runner_rerun', { FINDING_JSON: { finding_id: e.finding_id }, EDIT_JSON: e, CODE_DIR: INPUTS.code || '(none)', DATA_DIR: INPUTS.data || '(none)', RUN_DIR: (PATHS.session || '.') + '/phase2/runs/' + e.edit_id, SANDBOX_NOTES_PATH: PATHS.sandbox_notes || '' }), { ...GP, label: ('run:' + e.edit_id).slice(0, 56), phase: 'Implement', schema: RUNREC })
    }
    return { e, run }
  },
  async (prev, e) => {
    ...
    const scribe = await agent(promptRef('phase2/12_scribe_implementer', { EDIT_JSON: eWithTok, SOURCE_FILE_PATH: (INPUTS.source || '') + '/' + e.file, WORKING_BRANCH: 'paper-workshop/phase2', RULES_PATH: PATHS.rules || '' }), { ...GP, label: ('scribe:' + e.edit_id).slice(0, 56), phase: 'Implement', schema: SCRIBE })
    return { e: eWithTok, run: prev.run, scribe }
  },
  async (prev, e) => {
    const angles = anglesForEdit(e)
    const verdicts = (await parallel(angles.map(ang => () => agent(promptRef('05_verification_panel', { ANGLE: ang, ANGLE_QUESTION: ANGLE_Q[ang] || ('Judge from the ' + ang + ' angle.'), TARGET_JSON: { edit: prev.e, scribe_new_text: prev.scribe && prev.scribe.new_text, run: prev.run }, PAPER_TXT_PATH: (INPUTS.source || '') + '/' + e.file, STAGED_SOURCES_DIR: PATHS.staged_sources || '(none)', QUOTE_GATE_PATH: PATHS.quote_gate || '', RULES_PATH: PATHS.rules || '', RUBRIC_PATH: PATHS.rubric || '' }), { ...GP, label: ('vfy:' + ang + ':' + e.edit_id).slice(0, 56), phase: 'Verify', schema: VERIF })))).filter(Boolean)
...
const reconcile = await agent(promptRef('phase2/14_consistency_reconciler', { REVISED_SOURCE_PATH: (INPUTS.source || '') + '/(revised on branch paper-workshop/phase2)', RUN_ARTIFACTS_JSON: runArtifacts, BASELINE_NUMBERS_JSON: baseline.provenance_tokens || [], RUN_DIR: (PATHS.session || '.') + '/phase2/runs/reconcile' }), { ...GP, label: 'reconcile', phase: 'Reconcile', schema: RECON })
const reconcileClean = !reconcile.orphans.length && !reconcile.mismatches.length && !reconcile.run_mismatches.length
log('Reconcile: ' + (reconcileClean ? 'clean' : (reconcile.orphans.length + ' orphans, ' + reconcile.mismatches.length + ' mismatches, ' + reconcile.run_mismatches.length + ' run-mismatches')))

// ---------- Package ----------
phase('Package')
const pkg = await agent(promptRef('phase2/15_repro_package', { SESSION_PATH: PATHS.session || '.', INPUT_MANIFEST_JSON: INPUTS, RUN_RECORDS_JSON: results.map(r => r.run).filter(Boolean), REVISED_SOURCE_PATH: (INPUTS.source || '') + '/(branch paper-workshop/phase2)', PACKAGE_DIR: (PATHS.session || '.') + '/phase2/replication_package', SANDBOX_NOTES_PATH: PATHS.sandbox_notes || '' }), { ...GP, label: 'repro-package', phase: 'Package', schema: PACKAGE })
...
return {
  scope, baseline_status: baseline.status,
